...
choice = st.sidebar.selectbox("切換功能", ["📇 客戶資料卡", "➕ 新增客戶訂單", "💰 財務損益報表"])
...
if choice == "📇 客戶資料卡":
    ...
    if df_orders.empty:
        ...
        if filtered_df.empty:
            ...
            if matches.empty:
                st.error(f"找不到訂單編號: {target_oid}，請確認試算表格式或重新整理。")
            else:
                ...
                if not this_p.empty:
                    ...
                    with st.expander("📝 修改/刪除叫貨"):
                        sel_p = st.selectbox("選擇紀錄", this_p.index, format_func=lambda i: f"{this_p.loc[i, '廠商名稱']} ${this_p.loc[i, '進貨金額']}")
                        orig_p_idx = this_p.loc[sel_p, 'index']
                        p_c = st.number_input("修正金額", value=int(this_p.loc[sel_p, '進貨金額']), step=1)
                        p_n = st.text_input("修正備註", value=str(this_p.loc[sel_p, '備註']))
                        if st.button("💾 儲存進貨修改"):
                            df_purchases.loc[orig_p_idx, ["進貨金額", "備註"]] = [int(p_c), p_n]
                            conn.update(worksheet="採購明細", data=df_purchases)
                            st.success("已修正！"); st.rerun()
                        if st.button("🗑️ 刪除此進貨"):
