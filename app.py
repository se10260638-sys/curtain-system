...
def load_all():
    ...
    # 再次確認明細表是否有「訂單編號」欄位，沒有就補上 (防止 KeyError)
    if "訂單編號" not in df_d.columns:
        df_d["訂單編號"] = ""
    if "金額" not in df_d.columns:
        df_d["金額"] = 0
    ...
    if not df_orders.empty:
        ...
        if not f_df.empty:
            ...
            with st.form("add_det", clear_on_submit=True):
                ...
                other_name = st.text_input("自定義名稱 (選其他才填)") if f_name == "其他" else ""
...
# --- 功能 2：新增訂單 ---
    ...
    with st.form("new_order", clear_on_submit=True):
        ...
        if st.form_submit_button("✅ 建立訂單"):
            if not oid or not n_name: st.error("❌ 單號與姓名為必填！")
            elif clean_id(oid) in df_orders["訂單編號"].values: st.error("❌ 單號已存在！")
            else:
                new_row = pd.DataFrame([{"訂單編號": clean_id(oid), "訂單日期": str(n_date), "客戶姓名": n_name, "電話": n_phone, "地址": n_addr, "總金額": n_total, "已收金額": 0, "施工狀態": "已接單", "訂購內容": n_content}])
...
# --- 功能 3：損益中心 ---
    ...
    if pwd == ADMIN_PASSWORD:
        ...
        # --- 這裡加入安全檢查，避免 KeyError ---
        if not df_details.empty and "訂單編號" in df_details.columns:
            cost_sum = df_details.groupby("訂單編號")["金額"].sum().reset_index().rename(columns={"金額": "總支出"})
            cost_sum["訂單編號"] = cost_sum["訂單編號"].apply(clean_id)
            final_rpt = pd.merge(df_orders, cost_sum, on="訂單編號", how="left").fillna(0)
        else:
            final_rpt = df_orders.copy()
            final_rpt["總支出"] = 0
