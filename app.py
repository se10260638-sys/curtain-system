        ...
        if filtered_df.empty:
            ...
            if not this_p.empty:
                ...
                with st.expander("📝 修改或刪除叫貨記錄"):
                    sel_p_idx = st.selectbox("選擇要修正的記錄：", this_p.index, format_func=lambda i: f"{this_p.loc[i, '廠商名稱']} | ${this_p.loc[i, '進貨金額']}")
            ...
            with st.expander("➕ 新增叫貨"):
                ...
                final_v = p_v if p_v != "其他" else st.text_input("輸入自訂廠商")
...
# (其餘 新增訂單 與 財務報表 邏輯與之前一致，已確保穩定)
    ...
    with st.form("new_order", clear_on_submit=True):
        ...
        if st.form_submit_button("✅ 確認建檔"):
            if not n_name or not n_address: st.error("必填欄位請填寫")
            else:
                new_oid = f"ORD{datetime.now().strftime('%m%d%H%M%S')}"
                new_row = pd.DataFrame([{"訂單編號": new_oid, "訂單日期": str(datetime.now().date()), "客戶姓名": n_name, "電話": re.sub(r'\D', '', n_phone), "地址": n_address, "訂購內容": n_content, "總金額": int(n_total), "已收金額": int(n_paid), "師傅工資": 0, "施工狀態": "已接單", "代工師傅": n_worker}])
    ...
    if pwd == ADMIN_PASSWORD:
        ...
        st.metric("累積淨利", f"${int(report['淨利'].sum()):,.0f}")
