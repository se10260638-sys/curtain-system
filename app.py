...
# --- 網頁基本設定 ---
st.set_page_config(page_title="窗簾店管理系統 (尾款追蹤版)", layout="wide")
...
def load_data():
    ...
    # 自動補足舊資料缺少的欄位 (預防升級報錯)
    if "已收金額" not in df.columns:
        df["已收金額"] = df["總金額"] # 假設舊單都已收齊
...
df_temp['尾款'] = df_temp['總金額'] - df_temp['已收金額']
...
month_list = sorted(df_temp[df_temp['年份'] == selected_year]['月份'].unique().astype(int))
if not month_list: month_list = [datetime.now().month]
selected_month = st.sidebar.selectbox("選擇月份", month_list)

mask = (df_temp['年份'] == selected_year) & (df_temp['月份'] == str(selected_month))
...
if choice == "➕ 新增訂單":
    ...
    with st.form("add_form", clear_on_submit=True):
        ...
        if st.form_submit_button("儲存訂單"):
            ...
            save_data(df)
            st.success(f"訂單已儲存！目前尾款為: ${c_total - c_paid}")
            st.rerun()

# --- 功能 2：施工進度管理 ---
elif choice == "🏗️ 施工進度管理":
    st.header("工地進度追蹤")
    pending_df = df[df["狀態"] != "已收款"] # 只要沒收到錢都要追蹤
...
# --- 功能 3：修改/刪除訂單 ---
    ...
    if not filtered_df.empty:
        ...
        with st.form("edit_form"):
            ...
            with col_e1:
                e_name = st.text_input("客戶姓名", value=row["客戶姓名"])
                e_paid = st.number_input("已收金額 (更新此項以銷帳)", value=float(row["已收金額"]))
            ...
            with col_e2:
                ...
                e_worker = st.text_input("施工師傅", value=row["施工師傅"])
            
            e_content = st.text_area("內容", value=row["訂購內容"])
...
# --- 功能 4：💰 財務報表與尾款追蹤 (密碼保護) ---
    ...
    if pwd == ADMIN_PASSWORD:
        ...
        total_unpaid = filtered_df["尾款"].sum()
        c1.metric("當月總業績", f"${total_rev:,.0f}")
        c2.metric("已收現款", f"${total_paid:,.0f}")
        c3.metric("待收尾款", f"${total_unpaid:,.0f}", delta_color="inverse")
        
        st.divider()
        
        # 尾款催收專區 (顯示所有月份尚未付清的單子)
        st.subheader("⚠️ 全體未收齊尾款名單 (不限月份)")
        all_unpaid = df.copy()
        all_unpaid['待收尾款'] = all_unpaid['總金額'] - all_unpaid['已收金額']
        unpaid_list = all_unpaid[all_unpaid['待收尾款'] > 0]
        
        if not unpaid_list.empty:
            st.warning(f"目前共有 {len(unpaid_list)} 筆款項尚未收齊")
            st.dataframe(unpaid_list[["訂單日期", "客戶姓名", "電話", "總金額", "已收金額", "待收尾款", "狀態"]])
        else:
            st.success("太棒了！目前所有貨款皆已收齊。")
